...
public class SpaceStation {
    ...
    public SpaceStation(int maxCrew, int mapRows, int mapCols) {
        if (maxCrew <= 0) {
            throw new IllegalArgumentException("Max crew must be greater than zero. Got: " + maxCrew);
        }
        if (mapRows <= 0 || mapCols <= 0) {
            throw new IllegalArgumentException("Map dimensions must be greater than zero. Got: " + mapRows + "x" + mapCols);
        }
